...
@app.get("/")
def home():
    return {"status": "Backend Gemini dziala - extended nutrition v2"}
...
def apply_extra_fallbacks(result_json: dict) -> dict:
    ...
    saturated_fat = to_float(result_json.get("saturated_fat"), 0)
    ...
    # Fallback tłuszczów nasyconych:
    # jeżeli jest tłuszcz, to nasycone prawie nigdy nie powinny być zerem.
    ...
    # Fallback błonnika:
    # jeśli są węgle, szczególnie zboża/makaron/pieczywo, błonnik zwykle nie jest zerem.
    ...
    # Fallback soli:
    # większość gotowych dań, sosów, pizzy, kanapek i fast foodów ma sól.
    if salt <= 0:
